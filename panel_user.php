...
if (isset($_POST['validate_info'])) {
   ...
   if (empty($_POST['profil_email']) || !filter_var($_POST['profil_email'], FILTER_VALIDATE_EMAIL)) {
      ...
      // var_dump($_POST['profil_email']);
      if ($_SESSION['auth']->user_email != $_POST['profil_email']) {
         if ($user) {
   ...
   if (!password_verify($_POST['old_password'], $selectUser->user_password) && !empty($_POST['old_password'])) {
   ...
   if (empty($errors)) {
      $_SESSION['flash']['success'] = "Vos coordonnées on était modifiées";
   ...
   <div id="profil_user" class=" container d-flex flex-column align-items-center  py-5 mt-5">
      ...
      <div id="info_user" class=" d-flex flex-column align-items-center border border-dark border-2 rounded-3 my-3 py-4  px-3" style="background-color: #333533 ;color: #e8eddf;">
         ...
         <?php if (!$selectProfil->profil_default_reservation) :
            $displayNumberText = 'd-none' ?>
            <button class="btn" style="background-color: #cfdbd5;" id="add_number_reservation_default">Ajouter un nombre de réservation par default</button>
         <?php else :
            $displayNumberText = 'd-inline-block';
         ?>
         ...
         <label class="<?= $displayNumberText ?>" id="text_reservation">Nombre de réservation par defaut que vous avez configuré pour réserver</label>
         <p class="<?= $displayNumberText ?>" id="number_reservation"><?= $selectProfil->profil_default_reservation ?></p>
...
<script>
   ...
   let textReservationDefault = document.getElementById('text_reservation')
      ...
      if (number != "") {
         xhrUserNumber.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
               ...
               document.getElementById('number_reservation').textContent = test
            }
         }
   ...
   function insertNumberReservation(number, id) {
      ...
      if (number != "") {
         xhrUserNumber.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
               let test = xhrUserNumber.responseText
               let numberReservationInsert = document.getElementById('number_reservation')
               textReservationDefault.className = "d-inline-block"
               numberReservationInsert.className = "d-inline-block"
               numberReservationInsert.textContent = test
            }
         }
